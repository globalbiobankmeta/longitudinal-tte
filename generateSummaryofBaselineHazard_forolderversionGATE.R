...
option_list <- list(
  ...
  make_option("--eventTimeBinSize", type="numeric",default=1,
      help="event time is bin by eventTimeBinSize in step 1. Do not need to specify here if it is not specified in step 1"),
...
status <- modglmm$obj.glm.null$y

eventTime <- time[which(status == 1)]
